...
from sac_biped_walker import SAC, train_sac
...
def objective(trial: Trial) -> float:
    ...
    params = {
        ...
        'entropy_coef': trial.suggest_float('entropy_coef', 0.01, 1.0, log=True),
    ...
    seed = 0
    max_timesteps = 50_000  # Reduced for faster optimization
    eval_interval = 1_000
    n_eval_episodes = 5
    ...
    agent.actor_optimizer = torch.optim.Adam(agent.actor.parameters(), lr=params['actor_lr'])
    agent.critic_optimizer = torch.optim.Adam(agent.critic.parameters(), lr=params['critic_lr'])
    agent.target_entropy = -torch.prod(torch.Tensor([action_dim]).to(device)).item() * params['entropy_coef']
    ...
    for t in range(max_timesteps):
        ...
        if len(replay_buffer) > params['batch_size']:
            agent.train(replay_buffer, params['batch_size'], params['gamma'], params['tau'])
    ...
    # Return the best evaluation reward (Shouldn't this be average?)
...
def main():
    ...
    # Run optimization
    study.optimize(objective, n_trials=10, timeout=8 * 3600)  # 32 trials or 8 hour timeout
